...
const MatrimonialCard = ({ profile, index }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-36 bg-gradient-to-br from-pink-100 to-pink-50 flex items-center justify-center overflow-hidden">
        ...
        {!isAccepted && (
          <div className="absolute inset-0 bg-pink-100/60 backdrop-blur-md flex flex-col items-center justify-center">
            ...
            <p className="text-[9px] text-matrimonial-module/60 font-medium text-center px-4">Photo visible after<br/>interest accepted</p>
          </div>
        )}
        ...
        <div className="absolute bottom-2 left-2 bg-black/40 backdrop-blur-sm text-white text-[9px] px-2 py-0.5 rounded-full flex items-center gap-0.5">
      ...
      {/* Info */}
      <div className="p-2.5">
        <h3 className="font-semibold text-[13px] text-text-primary">{profile.name}, {profile.age}</h3>
        <p className="text-[10px] text-text-secondary">{profile.height} · {profile.gotra}</p>
        ...
        <button
          ...
          className={`w-full mt-2.5 py-2 rounded-xl text-[11px] font-semibold flex items-center justify-center gap-1 press-scale transition-all duration-200 ${
            interested
  ...
  return (
    <div className="min-h-screen bg-surface pb-20">
      ...
      <div className="bg-gradient-to-r from-matrimonial-module to-pink-400 px-4 pt-3 pb-5 rounded-b-3xl">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-white/70 text-[11px]">Find your match within the community</p>
          </div>
      ...
      <div className="px-4 mt-3 flex gap-2 overflow-x-auto scrollbar-hide pb-1" ref={scrollRef}>
        {filters.map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`shrink-0 px-3.5 py-1.5 rounded-full text-[11px] font-medium transition-all duration-200 press-scale ${
              filter === f
      ...
      <div className="px-4 mt-3 mb-3 flex items-center justify-between">
        <p className="text-[11px] text-text-secondary"><span className="font-semibold text-text-primary">{matrimonialProfiles.length}</span> profiles</p>
        <div className="flex items-center gap-1 text-[11px] text-matrimonial-module">
          <Sparkles size={11} />
